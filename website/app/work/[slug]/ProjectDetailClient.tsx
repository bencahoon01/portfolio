...
export default function ProjectDetailClient({ project }: { project: any }) {
  ...
  return (
    <main className="bg-background text-foreground min-h-screen">
      ...
      <div className="hidden lg:block sticky top-30 right-[8.33%] z-30 text-foreground font-mono text-lg font-bold float-right mr-[8.33%]">
        {new Date().getFullYear()}
      </div>
      <div className="hidden lg:block sticky top-30 left-[8.33%] z-30 text-foreground font-mono text-lg font-bold float-left ml-[8.33%]">
        EST. 2023
      </div>
      
      {/* Other decorative elements (non-sticky) */}
      <div className="hidden lg:flex fixed top-1/4 right-35 z-30 flex-col gap-2 text-foreground font-mono text-lg">
        <span>✕</span>
        <span>✕</span>
        <span>✕</span>
      </div>
      <div className="hidden lg:flex fixed bottom-1/4 left-35 z-30 flex-col gap-2 text-foreground font-mono text-lg">
        <span>✕</span>
        <span>✕</span>
        <span>✕</span>
      </div>
      <div className="hidden lg:block fixed bottom-8 left-35 z-30 text-foreground font-mono text-sm">
        <div>BEN CAHOON</div>
        <div>CENTRAL, SC</div>
        <div className="text-accent">AVAILABLE</div>
      </div>
      <div className="hidden lg:block fixed bottom-8 right-35 z-30 text-foreground font-mono text-sm text-right">
